...
final class SheetDetentsModelTests: XCTestCase {
  ...
  private func processDetentTest(sheetDetentsModel: SheetDetentsModel, detentTests: [DetentTest]) {
    for (height, direction, expectedDetentIdentifier) in detentTests {
      XCTAssertEqual(sheetDetentsModel.getDetent(forHeight: height, inDirection: direction), expectedDetentIdentifier, "getDetent(forHeight: \(height), inDirection: \(direction)) expected return of \(sheetDetentsModel.getDetent(forHeight: height, inDirection: direction)) but got \(expectedDetentIdentifier).")
    }
  }
