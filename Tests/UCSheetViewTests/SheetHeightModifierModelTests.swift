...
final class SheetHeightModifierModelTests: XCTestCase {
  ...
  func testSheetHeightModifierModel_Update_DefaultPan() {
    ...
    processPanTest(panValues: PanRecords.defaultPan.panValues, expectedValues: PanRecords.defaultPan.expectedSheetHeightModifierFromModifierModel)
  }
  ...
  func testSheetHeightModifierModel_Update_BottomCancelledPan() {
    sheetHeightModifierModel = SheetHeightModifierModel(sheetDetentsModel: sheetDetentsModel, initialSheetHeight: 100)
    processPanTest(panValues: PanRecords.bottomCancelledPan.panValues, expectedValues: PanRecords.bottomCancelledPan.expectedSheetHeightModifierFromModifierModel)
  }
  
  func testSheetHeightModifierModel_Update_ReversedMidPan() {
    sheetHeightModifierModel = SheetHeightModifierModel(sheetDetentsModel: sheetDetentsModel, initialSheetHeight: 100)
    processPanTest(panValues: PanRecords.reversedMidPan.panValues, expectedValues: PanRecords.reversedMidPan.expectedSheetHeightModifierFromModifierModel)
  }
  ...
  private func processPanTest(panValues: [PanValue], expectedValues: [SheetHeightModifier]) {
    ...
    for (translation, velocity, state) in panValues {
      let updatedSheetHeightModifierModel = sheetHeightModifierModel.update(translation: translation, velocity: velocity, state: state)
