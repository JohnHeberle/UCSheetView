...
final class SheetViewModelTests: XCTestCase {
  ...
  func testSheetViewModel_ModifiedDimming() {
    let sheetConfiguration = UCSheetView.Configuration(detents: detents, largestUnDimmedDetentIdentifier: .default, maxDimmingAlpha: 0.5)
    let viewModel = SheetViewModel(sheetConfiguration: sheetConfiguration)
  ...
  func testSheetViewModel_UpdateSheetPan() {
    processPanTest(panValues: PanRecords.defaultPan.panValues, expectedValues: PanRecords.defaultPan.expectedSheetHeightModifierFromViewModel)
  }
  
  func testSheetViewModel_UpdateSheetPan_TopCancelled() {
    processPanTest(panValues: PanRecords.topCancelledPan.panValues, expectedValues: PanRecords.topCancelledPan.expectedSheetHeightModifierFromViewModel)
  }
  
  func testSheetViewModel_UpdateSheetPan_BottomCancelled() {
    processPanTest(panValues: PanRecords.bottomCancelledPan.panValues, expectedValues: PanRecords.bottomCancelledPan.expectedSheetHeightModifierFromViewModel)
  }
  
  func testSheetViewModel_UpdateSheetPan_ReverseMidPan() {
    processPanTest(panValues: PanRecords.reversedMidPan.panValues, expectedValues: PanRecords.reversedMidPan.expectedSheetHeightModifierFromViewModel)
  }
